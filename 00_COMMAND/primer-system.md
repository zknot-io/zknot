...
-
NO this " `00_COMMAND/primers/ft260-provisioning.md` — first project primer (now "recipe proven" status)." is not correct, each separate concept will have a primer, and then one level up a primer and then an overall, never more than three primers per project, big picture primer/context. Infrastructe/system/platform primer for firmware, hardware, software, web, etc. That is for integration. and then each product has a primer, so WitnessMark, PowerVerify, Redoubt, verifyknot.io, etc
...
- Context Primers live in `~/ZKNOT/00_COMMAND/`. Commit them after edits (they're the load-bearing orientation
  layer; vault is single-copy on ~2-day manual backup).and each individual primer for any project or concept that is above a basic comprehension
